...
const DetailsBanner = ({ video, crew }) => {
  ...
  document.title = data ? data?.title || data?.name : "Serm_movies";

  const itemGenres = data?.genres.map((g) => g.id);

  const director = crew?.filter((f) => f.job === "Director");
  const writer = crew?.filter(
    (f) => f.job === "Screeenplay" || f.job === "Story" || f.job === "Writer"
  );
            ...
            <div className="w-full z-10 relative">
              <div className=" overflow-hidden absolute  opacity-10 top-0 left-0 w-full h-[650px] 3xl:h-[800px]">
                <Img
                  ...
                  className={`w-full object-cover h-[650px] 3xl:h-[800px]`}
                  alt="backdrop"
                />
              ...
              <div className="max-w-screen-xl text-white relative w-full flex flex-col md:flex-row gap-0 md:gap-10 mx-auto pt-[60px] px-4 md:px-6">
                <div className="py-3">
                  <Img
                    className=" max-w-full w-full xs:w-2/3 mx-auto md:w-[340px] rounded-lg"
                    src={
                      data?.poster_path
                ...
                <div className="py-3 w-full md:w-1/2">
                  ...
                  <div>
                    ...
                    <p className="w-full font-light mt-2 text-sm lg:text-base">
                      {data?.overview}
                    </p>
                  </div>
                  <div className="flex gap-5 mt-7 border-b pb-4 border-slate-700">
                    <div className="font-medium flex gap-2 items-center flex-wrap text-sm">
                      Status:
                      <span className="font-light flex items-center text-slate-400">
                        {data?.status}
                      </span>
                    </div>
                    <div className="font-medium flex gap-2 items-center flex-wrap text-sm">
                    ...
                    <div className="font-medium flex gap-2 items-center flex-wrap text-sm">
                      ...
                      <span className="font-light flex items-center text-slate-400">
                        {mediaType === "movie"
                          ? toHoursToMinutes(data?.runtime)
                          : toHoursToMinutes(data?.episode_run_time)}{" "}
                  ...
                  {director?.length > 0 && (
                    <div className="flex gap-5 mt-4 border-b pb-4 border-slate-700">
                      <div className="font-medium text-sm flex gap-2">
                        ...
                        {director?.map((d, i) => (
                          <span
                            className="font-light flex items-center text-slate-400"
                            key={i}
                          >
                            {d?.name}
                            {director.length - 1 !== i && ", "}
        ...
        <div className="w-full z-10 relative animate-pulse">
          <div className="max-w-screen-xl text-white relative w-full flex flex-col md:flex-row gap-0 md:gap-10 mx-auto pt-[60px] px-4 md:px-6">
            ...
            <div className="py-3 w-full md:w-1/2">
              ...
              <div className="my-5 flex gap-3 items-center">
                <div className=" w-16 md:w-20 aspect-1/1 bg-slate-800 rounded-full"></div>
                <div className=" flex items-center gap-3 cursor-pointer hover:text-red-500">
                  <div className="bg-slate-800 rounded-full aspect-1/1 w-16 md:w-20 "></div>
